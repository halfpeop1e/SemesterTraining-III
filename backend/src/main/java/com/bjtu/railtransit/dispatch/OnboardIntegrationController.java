...
public class OnboardIntegrationController {
    ...
    public OnboardIntegrationController(CommandBus commands, StatusFusion fusion,
        ...
        MovementAuthorityRegistry movementAuthorities) {
        this.commands=commands; this.fusion=fusion; this.events=events; this.simulation=simulation;
    ...
    public ApiResponse<String> status(@RequestBody StatusReport report) {
        fusion.accept(report); return ApiResponse.ok("status fused","ONBOARD_REPORTED");
    }
    ...
    public ApiResponse<String> event(@RequestBody OnboardEvent event) {
        events.accept(event); return ApiResponse.ok("event accepted",event.getEventId());
    }
    ...
    public ApiResponse<Map<String,Object>> onboard(@PathVariable String trainId) {
        ...
        data.put("train",train); data.put("commands",commands.forTrain(trainId));
    ...
    public ApiResponse<Map<String,Object>> workstation() {
        ...
        data.put("commands",commands.all()); data.put("pendingManualConfirmations",commands.pendingConfirmations());
        data.put("onboardReports",fusion.reports()); data.put("onboardEvents",events.events());
        ...
        data.put("protocolAdapterStatus",Map.of("signal","READY","vehicle","READY","driverDesk","READY","vision","READY"));
        return ApiResponse.ok("dispatcher workstation",data);
    }
    private double number(Object v) { return v instanceof Number n ? n.doubleValue() : 0; }
}
