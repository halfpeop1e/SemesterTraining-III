...
| Vision | `192.168.100.124:8303` UDP | 154 bytes, 92 signals / 40 switches |
...
python scripts/test_vision_154.py --capture 8303-port.pcapng
```
...
The Vision 154-byte table is not available in the project. The Vision script
therefore does not generate a packet. It can replay a previously captured
154-byte packet only after explicit approval and only when Windows selects a
`192.168.100.x` source address:

```powershell
python scripts/test_vision_154.py --capture 8303-port.pcapng --send --teacher-approved
```
