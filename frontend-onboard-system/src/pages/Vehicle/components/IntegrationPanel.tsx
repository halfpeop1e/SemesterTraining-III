...
export default function IntegrationPanel({
  ...
  if (!deviceIdRef.current) {
    ...
    deviceIdRef.current =
      sessionStorage.getItem(key) || `HMI-${trainId}-${crypto.randomUUID().slice(0, 8)}`;
  ...
  }, [snapshot, localState, pageStatus, departureAuthorized, onDepartAuthorized]);

  useEffect(() => {
    if (!localState || !['playing', 'finished'].includes(pageStatus)) return;
    const action = snapshot?.commands.find(
      (command) =>
        ['FORCE_HOLD', 'HOLD', 'EMERGENCY_RECOVERY'].includes(command.commandType) &&
        ['PENDING', 'CONFIRMED'].includes(command.status),
    );
  ...
  const reportCurrent = async () => {
    ...
    await reportOnboardStatus({
      ...
      positionMeters: state.absolutePosition ?? lineStartPosition + state.position,
  ...
  useEffect(() => {
    if (!localState || !['playing', 'finished'].includes(pageStatus)) return undefined;
    void reportCurrent();
  ...
  return (
    <section className="integration-panel" aria-label="总控联调">
      ...
      <div>
        线路 {fromStationName} → {toStationName} ·
        速度 {train ? (train.velocity * 3.6).toFixed(1) : '0.0'} km/h ·
        位置 {absolutePosition.toFixed(0)} m ·
        限速 {snapshot?.speedLimitKmh?.toFixed(0) ?? '0'} km/h ·
        MA {snapshot?.movementAuthorityMeters?.toFixed(0) ?? '0'} m
      </div>
      <div>
